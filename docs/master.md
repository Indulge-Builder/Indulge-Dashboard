...
| **deals** | Zoho deals (ex `onboarding_deals`) | `deal_id` PK · `deal_name` · `agent_name` · `created_at` (entry time) · `closing_date` (date, Zoho Closing_Date — the day the closure counts on; migration 20260907120000); PK violation 23505 = expected dedup |
...
| `/api/webhooks/zoho-deals` | POST | Insert `deals` only (deduped on `deal_id`; 23505 silently ignored); accepts `closing_date` (ISO / dd/MM/yyyy / datetime → IST day) and retries without it if the column is not migrated yet. The old `onboarding_conversion_ledger` dual-write was removed — the table is orphaned (dry-audit G4, drop/archive decision pending) |
